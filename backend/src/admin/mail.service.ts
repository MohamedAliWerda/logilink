...
export class MailService {
  ...
  async sendFeedbackReminder(to: string, companyName: string, postTitle: string, feedbackUrl: string): Promise<void> {
    ...
    const subject = `LogiLink – Votre retour sur l'offre : ${postTitle}`;

    const html = `
      <div style="font-family: Arial, Helvetica, sans-serif; line-height: 1.6; color: #1f2937; max-width: 600px; margin: 0 auto;">
        <div style="background: #f97316; padding: 24px 32px; border-radius: 8px 8px 0 0;">
          <h1 style="color: #fff; margin: 0; font-size: 20px;">LogiLink – Section Feedback disponible</h1>
        </div>
        <div style="background: #fff; padding: 32px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 8px 8px;">
          <p>Bonjour <strong>${this.escapeHtml(companyName)}</strong>,</p>
          <p>
            Cela fait maintenant plus de <strong>6 mois</strong> que vous avez publié l'offre
            <em>« ${this.escapeHtml(postTitle)} »</em> sur la plateforme <strong>LogiLink</strong>.
          </p>
          <p>
            Dans le cadre du suivi pédagogique de l'ISGIS, une section <strong>Feedback</strong> est désormais
            disponible dans votre espace. Nous vous invitons à la remplir afin de nous aider à améliorer
